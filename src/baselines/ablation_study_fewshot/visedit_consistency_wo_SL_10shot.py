...
from prompts.ablation_study_10shot.visedit_consistency_wo_SL_prompt import CLAUSE_LOCATION_PROMPT, GENERATION_PROMPT, MERGE_PROMPT
...
You will be given a user utterance, the database schema, an incorrect data visualization query and the corresponding guidance that helps you correct the incorrect query to the correct one.
...
You will be given a user utterance, the database schema, an incorrect data visualization query, the identified wrong clauses, the corresponding guidance that helps you correct the incorrect query to the correct one.
...
class Model:
    ...
    def generate(self, args, df_row):
        ...
        for _ in range(self.args.top_k):
            # clause location
            user_prompt_clause_location = CLAUSE_LOCATION_USER_PROMPT.format(question=question, err_query=err_query, feedback=feedback, schema=schema, template=json.dumps(clause_location_template))
            messages = [
            ...
            # clause Generation
            user_prompt_clause_generation = GENERATION_USER_PROMPT.format(question=question, err_query=err_query, schema=schema, wrong_clause=wrong_clause, feedback=feedback, template=json.dumps(generation_template))
